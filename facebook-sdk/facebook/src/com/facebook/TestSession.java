 ...
 * <p/>
 * The main use case for this class is using {@link #createSessionWithPrivateUser(android.app
 * .Activity, java.util.List)}
 ...
 * is made to find an existing test user that has the required permissions. If no such user is
 * available,
...
public class TestSession extends Session {
     ...
     * close; This method should not be used in application code -- but is useful for creating
     * unit tests
     ...
     * creating one if necessary on open (but not deleting it on close,
     * so it can be re-used in later
     * tests).
     * <p/>
     ...
     *
     * @param activity             the Activity to use for opening the session
     * @param permissions          list of strings containing permissions to request; nil will
     *                             result in
     *                             a common set of permissions (email,
     *                             publish_actions) being requested
     * @param sessionUniqueUserTag a string which will be used to make this user unique among other
     *                             users with the same permissions. Useful for tests which
     *                             require two or more users to interact
     *                             with each other, and which therefore must have sessions
    ...
    private static synchronized TestSession createTestSession(Activity activity,
                                                              List<String> permissions, Mode mode,
                                                              String sessionUniqueUserTag) {
        if (Utility.isNullOrEmpty(testApplicationId) || Utility.isNullOrEmpty
    ...
    private static synchronized void retrieveTestAccountsForAppIfNeeded() {
        ...
        String testAccountQuery = String.format("SELECT id,access_token FROM test_account WHERE " +
                        "app_id = %s",
                testApplicationId);
        ...
        // Build a JSON string that contains our queries and pass it as the 'q' parameter of the
        // query.
    ...
    private static synchronized void populateTestAccounts(Collection<TestAccount> testAccounts,
        ...
        // We get different sets of data from each of these queries. We want to combine them into
        // a single data
    ...
    public final String toString() {
        ...
        return new StringBuilder().append("{TestSession").append(" testUserId:").append
                (testAccountId)
    ...
    void forceExtendAccessToken(boolean forceExtendAccessToken) {
        ...
        setTokenInfo(
                new AccessToken(currentToken.getToken(), new Date(), currentToken.getPermissions(),
                        currentToken.getDeclinedPermissions(), AccessTokenSource.TEST_USER,
                        new Date(0))
        );
    ...
    private void findOrCreateSharedTestAccount() {
        TestAccount testAccount = findTestAccountMatchingIdentifier
    ...
    private TestAccount createTestAccountAndFinishAuth() {
        ...
        // in another shared session. If we're in private mode, don't bother renaming it since
        // we're just going to
        ...
        if (error != null) {
            ...
            // If we are in shared mode, store this new account in the dictionary so we can
            // re-use it later.
            if (mode == Mode.SHARED) {
                // Remember the new name we gave it, since we didn't get it back in the results
    ...
    private void deleteTestAccount(String testAccountId, String appAccessToken) {
        ...
        } else if (graphObject.getProperty(Response.NON_JSON_RESPONSE_PROPERTY) == (Boolean)
                false) {
    ...
    private String getSharedTestAccountIdentifier() {
        ...
        long sessionTagHash = (sessionUniqueUserTag != null) ? sessionUniqueUserTag.hashCode() &
                0xffffffffL : 0;
